...
def run_nmap_scan(ip: str) -> tuple:
    ...
    try:
        nm = nmap.PortScanner()
        evasion_args = (
            "-sV -f -D RND:5 --data-length 20 "
            "--version-intensity 3 -T4 --script vuln"
        )
        nm.scan(ip, arguments=f"{evasion_args} -p 21,22,80,443,3306,8080,8443")
...
def generate_pdf_report(report_data: dict, domain: str) -> str:
...
def main():
    if os.geteuid() != 0:
        console.print(
            "[bold red][!] This tool requires root privileges. Run with sudo.[/bold red]"
        )
        sys.exit(1)
    ...
    pdf_file = generate_pdf_report(report_data, domain)
    if pdf_file:
